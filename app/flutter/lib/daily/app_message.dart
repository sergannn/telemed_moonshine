...
class AppMessage with _$AppMessage {
  ...
  const factory AppMessage.chatMessage({required ChatMessage message}) =
      _ChatMessage;

  const factory AppMessage.chatMessageReaction(
      {required ChatMessageReaction reaction}) = _ChatMessageReaction;
  ...
  factory AppMessage.fromJson(Map<String, dynamic> json) {
    ...
    if (event == 'message-reaction') {
      return AppMessage.chatMessageReaction(
        ...
        reaction:
            ChatMessageReaction.fromJson(json..['message']['local'] = false),
      );
    }
