...
class _RemoteParticipantViewState extends State<RemoteParticipantView> {
  ...
  double localSize = 1.0; // Default full size
  ...
  Widget build(BuildContext context) {
    ...
    final canAdminLiveStreams =
        canAdmin?.contains(CanAdminPermission.streaming) ?? false;

    final localPermissions = participants.local.info.permissions;
    final localIsParticipantAdmin =
        localPermissions?.canAdmin.contains(CanAdminPermission.participants) ??
            false;
    final localIsOwner = participants.local.info.isOwner;

    final isScreenSharing = participant?.media?.screenVideo.track != null;
    final videoFit =
        isScreenSharing ? VideoViewFit.contain : VideoViewFit.cover;

    final isMicrophoneOn =
        participant?.media?.microphone.state != MediaState.off;
    final isCameraOn = participant?.media?.camera.state != MediaState.off;
    final isScreenShareOn =
                ...
                child: PopupMenuButton<void Function()>(
                  ...
                  itemBuilder: (context) => [
                    ...
                    PopupMenuItem(
                      ...
                      child: ListTile(
                        leading:
                            Icon(hasPresence ? Icons.hide_image : Icons.image),
                        title: Text(hasPresence ? 'Hide' : 'Show'),
                      ),
                    ),
                    PopupMenuItem(
                      value: () => _setParticipantCanAdminLiveStreams(
                          !canAdminLiveStreams),
                      child: ListTile(
                        leading: Icon(
                            canAdminLiveStreams ? Icons.tv_off : Icons.live_tv),
                        title: Text(canAdminLiveStreams
                            ? 'Demote live stream admin'
                            : 'Make live stream admin'),
                      ),
                    ),
                    if (isMicrophoneOn)
                      PopupMenuItem(
  ...
  void _setParticipantCanSendMic(bool canSend) {
    final Set<CanSendPermission> permissions = {
      ...widget.client.participants.all[widget.participantId]?.info.permissions
              ?.canSend ??
          {}
    };
    if (canSend) {
