...
class _PersistentTabViewState extends State<PersistentTabView> {
  ...
  Widget _buildScreen(int index) => CustomTabView(
    key: _tabKeys[index],
    navigatorConfig: widget.tabs[index].navigatorConfig.copyWith(
      ...
      return PopScope(
        ...
        child: NotificationListener<NavigationNotification>(
          onNotification: (notification) {
            final newCanPop = calcCanPop(
              subtreeCantHandlePop: !notification.canHandlePop,
            );
            if (newCanPop != canPop) {
              setState(() {
                canPop = newCanPop;
